...
| `WFD_保底字庫.html` / `WFD_易錯單字庫.html` | 防漏打功能字／易錯字參考 |
...
- **目前有未 commit 的修改**：`SST_擬答.html` 和 `WE_論點總表.html` 各有數百行 diff（2026-07 sessions 的基礎字簡化＋擬答改寫），接手先 `git status` 確認要不要先 commit push。
- git 一律 `git -C /Users/nn/Documents/claude/pte-study`（Bash cwd 會跳走，曾差點 amend 錯 repo）；`git add -A` 前先看 status，別掃進無關檔案。
