...
| `WE_漫畫.html` | **原型（2026-09-13）**：每題 10 格漫畫（P1 3 格 topic/va/vb、P2 3 格 con+conR/cond+resA/resB+concl、P3 3 格 stance+reason/does/result+impl、P4 1 格 final+rec），對話框只放 slot 不放模板。**沒有自己的題庫**：fetch 總表 DATA/ZH（同背誦地圖）。圖是 SVG 線稿：`SPR` 元件庫（人物 face()+body(pose) 組合、建築、道具、`bg*` 背景），`scene()` 把 `[bg, [元件,x,y,縮放,{flip,t}]...]` 擺到 300×150 畫布；`PIC[題號]` 每題 10 個場景，只畫了 ①-1、①-2，其餘走 `GENERIC`。默寫模式遮 slot 留首字母、點格揭開 |
