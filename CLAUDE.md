...
| `SWT_擬答.html` | SWT 179 題擬答（2026-09-06）。James 定的規則：從紅字句挑 3-4 句、**只刪不改字**、用 and/but 拼成一句、50-60 字；每題換 3-4 個基礎字防複製貼上（綠字＋括弧原字）。DATA 陣列驅動，含原文黃底標示、中文翻譯、機經錯字提醒。加題／改題用 scratchpad 的 `swt_add.py` 驗證器（來源句必須是原文子字串、擬答扣掉換字後須等於來源句刪減拼接） |
