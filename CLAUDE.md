...
| `SWT_四格漫畫.html` | SWT 179 題四格漫畫（2026-09-24，James 考前沒時間複習，要「看完漫畫和句子就知道這篇在講什麼」；09-25 他嫌線稿醜，改成彩色 Twemoji＋場景背景；再要求「像日本漫畫」→ 加網點、but 格集中線、emoji 墨線描邊、Q 版角色對話框（`avatar.js` 的 `MOODS` 8 種表情，`moodOf()` 依 and/but 與徽章挑）、音效字（`sfxOf()`：只有 bang→BOOM!／x→NO!／q→?／zz→Zzz 四種，英文；第一版用日文擬音且每格都有，James 說「太多、看不懂日文，只能看英文或中文」，「音效」鈕可關）、「第 N 話」標題、Dela Gothic One 字型走 Google Fonts；他再拿 Slam Dunk 圖問能不能更精細，選了「人物改自繪動漫風、物件背景維持 emoji」→ `avatar.js` 的 `figure()` 產生器：`HAIR` 髮型（boy/girl/old/kid）＋`HAT`（cap/hardhat/lamp/straw/khaki/crown/helmet/grad/glasses）＋`MOODS` 表情＋`POSES` 姿勢（stand/sad/point/wave/cheer/cross/think/reach/run/sit/shake）＋`OUTFIT` 服裝，`FIG` 定義 40 種人物、`group()` 組多人（crowd/family/shake…），`people.js` 把 EMO 的人物鍵覆蓋成 `{fig:svg}`；真要 Slam Dunk 等級得靠生圖模型，他不想給 API key）。**沒有自己的題庫**：fetch `SWT_擬答.html` 抽 `const DATA`（本機要 http server）。一格＝擬答裡的一句來源句：`splitPanels()` 把 `ans` 依 `src` 各句字數對齊切格（`{and}`/`{but}` 不計字），對不齊（#154 機經黏字）就退回照連接詞切；56 題 3 格、119 題 4 格、4 題 5 格。格與格之間的 `and`（藍）/`but`（紅）用不同色塊標，整句列再拼一次。**圖**：`PIC[題號] = [[icons, 藍字, 中文, (可選)背景], ...]` 一格一筆，icons 1–5 個元件鍵（281 個鍵，`EMO` 表對到 Twemoji：單一 emoji／`{e,t}` emoji 疊字／`{t}` 純文字／`{parts}` 多 emoji 組合），`icon:badge` 右上疊徽章（`BADGE`：x/up/down/q/ok/no/heart/zz/sweat/star/bang/dollar/idea/time；up/down 用 hue-rotate 變綠／紅）；`scene()` 依元件數自動排版落在地平線上並畫影子；背景由 `BGS` 依格內元件自動挑（space/deep/sea/snow/night/desert/forest/farm/lab/clinic/class/home/office/city/sport/out，都沒命中＝paper 點格紙），順序＝優先權。emoji 圖檔自架在 `img/emoji/*.svg`（Twemoji 15.1，CC-BY 4.0，頁尾有註明；檔名＝twemoji 規則：無 ZWJ 就去掉 FE0F 的 codepoint 串），新增 emoji 要把檔案抓進來（`https://cdn.jsdelivr.net/gh/jdecked/twemoji@15.1.0/assets/svg/<code>.svg`），載入失敗會退回系統 emoji 字。默寫（遮英文留首字母，點格揭開）／藏中文／藏原字／連讀（179 題一頁）；`#q{num}` 直達（num＝擬答題號）。加題：擬答加 DATA 後，這頁加一筆 PIC（格數＝src 句數） |
